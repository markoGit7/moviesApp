...
import {recommedationShowsByID, Genres} from '../api/movies.js'
...
function RecomendedShows({show_id}) {
    ...
    useEffect(() => {//fetching movies
        ...
        (async () => {
            const result = await recommedationShowsByID(show_id);
            const mostFamous_Existing_7 = result.results.sort((a, b) => b.popularity - a.popularity).filter(row => row.poster_path !== null).slice(0, 8);
            setShows(mostFamous_Existing_7);
            

            console.log('Recomendations Shows', mostFamous_Existing_7);
